...
from final_score import Ui_Dialog as sc     #importing final score file
import sqlite3
...
"font: 75 italic 11pt \"Century Gothic\";")
        ...
        self.calcscore_btn.setGeometry(QtCore.QRect(230, 430, 141, 31))
        font = QtGui.QFont()
        font.setFamily("MS Shell Dlg 2")
        ...
        #when calculate score button clicked
    ...
    def retranslateUi(self, evaluate_team):
        ...
        x=cur.execute("SELECT DISTINCT Name FROM teams;")
        data=x.fetchall()   #fetching the data from teams table
        for i in data:
    ...
    def changedname(self,t):
        ...
        y = cur.execute("SELECT Players from teams WHERE Name='" + t + "';")
        player = y.fetchall()   #fetching the data from teams table
        for j in player:
            self.players_lw.addItem(j[0])
        z = cur.execute("SELECT Value from teams WHERE Name='" + t + "';")
        value = z.fetchall()    #fetching the data from teams table
        for k in value:
            self.scores_lw.addItem(str(k[0]))



    def final_score(self):      #function for final score 
        total_score=0
        ...
        for k in value:
            total_score+=k[0]
        self.score_screen.Net_score.setText(str(total_score))  
